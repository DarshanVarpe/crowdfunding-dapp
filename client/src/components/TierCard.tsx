...
// --- FIX #1: Add the new prop to the type definition ---
type TierCardProps = {
    tier: Tier;
    index: number;
    ...
    onActionSuccess: () => void; // This line was missing
};

// --- FIX #2: Accept the new prop in the component's function signature ---
export const TierCard: React.FC<TierCardProps> = ({ tier, index, contractAddress, isEditing, campaignStatus, campaignOwner, onActionSuccess }) => {
    ...
    const handleFund = async () => {
        ...
        try {
            ...
            onActionSuccess(); // Use the new prop instead of reloading
        } catch (error: any) {
    ...
    // Determine if the funding button should be disabled
    const isOwner = activeAccount?.toLowerCase() === campaignOwner.toLowerCase();
    const isNotActive = campaignStatus !== 0; // 0 is 'Active' state
    const canFund = !isOwner && !isNotActive;
    
    // Determine the text for the disabled button
        ...
        <div className="w-full max-w-sm flex flex-col justify-between p-6 bg-white border border-gray-200 rounded-lg shadow-md transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
            <div className="mb-5">
                ...
                <p className="text-3xl font-extrabold text-blue-600 mt-2">
                    {ethers.formatEther(tier.amount)} <span className="text-lg font-medium text-gray-500">POL</span>
                </p>
            </div>
            ...
            <div className="flex flex-col mt-auto space-y-3">
                <div className="flex justify-between items-center">
                    <p className="text-sm font-medium text-gray-600">
                        <span className="font-bold text-gray-800">{tier.backers.toString()}</span> Backer(s)
                    </p>
                    <button
                        onClick={handleFund}
                        disabled={isLoading || !canFund}
                        className="px-5 py-2 text-sm font-semibold text-white bg-blue-600 rounded-md shadow-sm hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                        title={!canFund ? disabledButtonText : "Fund this tier"}
                    >
                        {isLoading ? (
                ...
                {isEditing && (
                    <button
                        ...
                        disabled={isLoading}
                        className="w-full px-4 py-2 bg-red-600 text-white text-sm font-semibold rounded-md hover:bg-red-700 disabled:bg-gray-400 transition-colors"
                    >
